...
@register_kernel("bitsandbytes::bf16_mitchell_a_matmul", "cuda")
def _(A: torch.Tensor, B: torch.Tensor, M: int, N: int, K: int) -> torch.Tensor:
    out = torch.empty((M, N), dtype=torch.bfloat16, device=A.device)
    torch._check(A.dtype == torch.bfloat16, lambda: f"A must be bfloat16, got {A.dtype}")
    torch._check(B.dtype == torch.bfloat16, lambda: f"B must be bfloat16, got {B.dtype}")
    ...
    torch._check(B.shape == (N, K), lambda: f"B.shape must be ({N}, {K}), got {B.shape}")
    with _cuda_device_of(A):
        lib.cbf16_mitchell_a_matmul(get_ptr(A), get_ptr(B), get_ptr(out), ct.c_int(M), ct.c_int(N), ct.c_int(K), _get_tensor_stream(A))
    stream = torch.cuda.current_stream(A.device)
    A.record_stream(stream)
    B.record_stream(stream)
    return out
...
@register_kernel("bitsandbytes::bf16_mitchell_b1_matmul", "cuda")
def _(A: torch.Tensor, B: torch.Tensor, M: int, N: int, K: int) -> torch.Tensor:
    out = torch.empty((M, N), dtype=torch.bfloat16, device=A.device)
    torch._check(A.dtype == torch.bfloat16, lambda: f"A must be bfloat16, got {A.dtype}")
    torch._check(B.dtype == torch.bfloat16, lambda: f"B must be bfloat16, got {B.dtype}")
    ...
    torch._check(B.shape == (N, K), lambda: f"B.shape must be ({N}, {K}), got {B.shape}")
    with _cuda_device_of(A):
        lib.cbf16_mitchell_b1_matmul(get_ptr(A), get_ptr(B), get_ptr(out), ct.c_int(M), ct.c_int(N), ct.c_int(K), _get_tensor_stream(A))
    stream = torch.cuda.current_stream(A.device)
    A.record_stream(stream)
    B.record_stream(stream)
    return out
